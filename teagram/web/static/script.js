function showNotification(title, text, color = "greenyellow") {
...
function updating_qr(){
  ...
  if (__qr) {
      ...
      .then(
          ...
          (error) => {showNotification("ERROR", error); clearInterval(_interval)}
      )
      if (__qr && (tries == 10)){
          tries = 0
...
document.getElementById("enter").onclick = async () => {
  if (!_2fa){
    ...
    if (!_id || !_hash){
      showNotification("Error", "You didn't enter api_id or api_hash", "red")
      return
    }
    ...
    if (__qr){
      showNotification("Error", "You didn't scan QRCode", "red")
      return;
    }
    ...
    if (!passwd){
      showNotification("Error", "You didn't enter 2FA password", "red")
      return
    }
    ...
    try {
      ...
      showNotification("Unexpected error", error, 'red')
    }
  }  
}